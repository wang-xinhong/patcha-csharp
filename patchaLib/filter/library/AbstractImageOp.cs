...
namespace org.patchca.filter.library
{
	...
	public abstract class AbstractImageOp : BufferedImageOp
	{

		public const int EDGE_ZERO = 0;
		public const int EDGE_MIRROR = 1;
		public const int EDGE_CLAMP = 2;

		protected internal int edgeMode;
		protected internal RenderingHints hints;
		...
		protected internal virtual int getPixel(int[] pixels, int x, int y, int width, int height, int edgeMode)
		{
		...
		protected internal virtual int bilinear(int nw, int ne, int sw, int se, double xd, double yd)
		{
			return linear(linear(nw, ne, xd), linear(sw, se, xd), yd);
		}

		protected internal virtual int getPixelBilinear(int[] pixels, double x, double y, int width, int height, int edgeMode)
		{
		...
		protected internal virtual int limit(int v, int min, int max)
		{
			if (v < min)
			{
				v = min;
		...
		protected internal virtual int limitByte(int v)
		{
			return limit(v, 0, 255);
		}

		protected internal virtual void filter(int[] inPixels, int[] outPixels, int width, int height)
		{
